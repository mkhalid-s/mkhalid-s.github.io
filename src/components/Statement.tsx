...
export default function Statement({ segments, activeId, onToggle }: Props) {
  ...
  return (
    <h1 className="max-w-2xl font-display text-[2.1rem] font-normal leading-[1.3] tracking-[-0.01em] text-ink sm:text-[3rem] sm:leading-[1.2] md:text-[3.6rem] lg:text-[4.25rem] lg:leading-[1.1] lg:tracking-[-0.02em]">
      {segments.map((s, i) =>
        s.t === 'text' ? (
          <m.span key={i} {...anim(i)}>
            {s.v}
