...
export default function Statement({ segments, activeId, onToggle }: Props) {
  ...
  return (
    <h1 className="max-w-3xl font-display text-[2.25rem] font-normal leading-[1.25] tracking-[-0.01em] text-ink sm:text-[3.1rem] sm:leading-[1.15] md:text-[3.8rem] lg:text-[4.5rem] lg:leading-[1.08] lg:tracking-[-0.025em]">
      {segments.map((s, i) =>
        s.t === 'text' ? (
          <m.span key={i} {...anim(i)}>
            {s.v}
